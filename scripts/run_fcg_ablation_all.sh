#!/usr/bin/env bash
#
# run_pg_ablation_all.sh
#
# Batch-run ablations/run_eval_fcg_sentiment_ablation.py across every model and every
# ablation method on SST2.  Each run writes a log under logs/pg_ablation/
# and the final metrics are aggregated into a single CSV.
#
# Usage:
#   bash run_pg_ablation_all.sh                      # all combos, default args
#   bash run_pg_ablation_all.sh --steps 50 --topk 10 # override extra args
#
...
DATASETS=( sst2 )                                # SST2 only as requested
METHODS=(  l1         l2      scalar   )

SCRIPT="ablations/run_eval_fcg_sentiment_ablation.py"
LOG_DIR="logs/pg_ablation"
CSV_FILE="${LOG_DIR}/results.csv"

# Optional: pass extra args to every run, e.g. --steps 50 --n-samples 500
EXTRA_ARGS="${@}"
...
# CSV header
echo "model,dataset,method,samples,log_odds,comprehensiveness,sufficiency,soft_log_odds,soft_comp,soft_suff,avg_time_s" \
    > "$CSV_FILE"

echo "=========================================================================="
echo "  PACE Gradient — Ablation Batch Runner"
echo "=========================================================================="
echo "  Models    : ${MODELS[*]}"
...
for model in "${MODELS[@]}"; do
    for dataset in "${DATASETS[@]}"; do
        for method in "${METHODS[@]}"; do
            ...
            CURRENT=$((CURRENT + 1))
            LOGFILE="${LOG_DIR}/${model}_${dataset}_${method}.log"

            printf "[%3d/%3d]  model=%-10s  dataset=%-6s  method=%-6s  →  %s\n" \
                   "$CURRENT" "$TOTAL" "$model" "$dataset" "$method" "$LOGFILE"
            ...
            if python "$SCRIPT" \
                ...
                #  Append CSV row 
                echo "${model},${dataset},${method},${SAMPLES:-NA},${LO:-NA},${COMP:-NA},${SUFF:-NA},${SLO:-NA},${SCOMP:-NA},${SSUFF:-NA},${AVGT:-NA}" \
                    >> "$CSV_FILE"

                echo "         ✓   samples=${SAMPLES:-?}  LO=${LO:-NA}  Comp=${COMP:-NA}  Suff=${SUFF:-NA}  avgT=${AVGT:-NA}s"
            else
                echo "         ✗   FAILED  (see $LOGFILE)"
                echo "${model},${dataset},${method},FAILED,,,,,,," >> "$CSV_FILE"
                FAILED=$((FAILED + 1))
            fi
