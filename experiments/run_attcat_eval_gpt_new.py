"""
run_attcat_eval_gpt_new.py

Benchmark AttCAT attribution on decoder-only GPT models using the three
...
Usage examples:
    python run_attcat_eval_gpt_new.py --experiment tellmewhy --model_name gpt2
    python run_attcat_eval_gpt_new.py --experiment longra    --model_name gpt2
    python run_attcat_eval_gpt_new.py --experiment wikibio   --model_name gpt2-medium
    python run_attcat_eval_gpt_new.py --experiment tellmewhy --model_name facebook/opt-1.3b
    python run_attcat_eval_gpt_new.py --experiment longra    --model_name EleutherAI/gpt-j-6b --num_samples 36
"""
...
from attcat_gpt2 import attcat_gpt2
...
def run_single_example(
    ...
    """
    Run AttCAT attribution and compute faithfulness metrics for one sample.
...
def run_benchmark(args) -> None:
    ...
    # Register in attcat_gpt2's cache to avoid reloading on every call
    from attcat_gpt2 import _CACHE as _ac_cache
    _ac_cache[(args.model_name, device)] = (model, tokenizer)
    ...
    for idx, sample in enumerate(tqdm(samples, desc=f"[AttCAT/{experiment}]")):
        try:
...
def _print_sample(question: str, res: dict):
    ...
    print("Top-5 Q tokens by AttCAT attribution:")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Benchmark AttCAT Attribution on GPT-family models "
...
""",
    ...
    parser.add_argument(
        ...
        help="Baseline for AttCAT base_embed (method reference, default: zero)",
    )
    parser.add_argument(
