"""
run_eval_ig_gpt_new.py

Benchmark Vanilla Integrated Gradients attribution on decoder-only GPT models
using the three evaluation settings from Zhao & Shan (ReAGent, AAAI 2024):
...
Usage examples:
    python run_eval_ig_gpt_new.py --experiment tellmewhy --model_name gpt2
    python run_eval_ig_gpt_new.py --experiment longra    --model_name gpt2
    python run_eval_ig_gpt_new.py --experiment wikibio   --model_name gpt2-medium
    python run_eval_ig_gpt_new.py --experiment tellmewhy --model_name facebook/opt-1.3b
    python run_eval_ig_gpt_new.py --experiment longra    --model_name EleutherAI/gpt-j-6b --num_samples 36
"""
...
def run_single_example(
    ...
    """
    Run Vanilla IG attribution and compute faithfulness metrics for one sample.
...
def run_benchmark(args) -> None:
    ...
    print("=" * 60)
    print(f"Method        : Vanilla Integrated Gradients")
    ...
    # Register in ig_gpt2's cache to avoid reloading on every call
    from ig_gpt2 import _CACHE as _ig_cache
    _ig_cache[(args.model_name, device)] = (model, tokenizer)
    ...
    for idx, sample in enumerate(tqdm(samples, desc=f"[VanillaIG/{experiment}]")):
        try:
...
def _print_sample(question: str, res: dict):
    ...
    print("Top-5 Q tokens by IG attribution:")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Benchmark Vanilla IG Attribution on GPT-family models "
