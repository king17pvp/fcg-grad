"""
run_eval_pg_gpt_new.py

Benchmark FCGGrad Attribution on decoder-only GPT models using the
...
Usage examples:
    python run_eval_pg_gpt_new.py --experiment tellmewhy --model_name gpt2
    python run_eval_pg_gpt_new.py --experiment longra    --model_name gpt2
    python run_eval_pg_gpt_new.py --experiment wikibio   --model_name gpt2-medium
    python run_eval_pg_gpt_new.py --experiment tellmewhy --model_name facebook/opt-1.3b
    python run_eval_pg_gpt_new.py --experiment longra    --model_name EleutherAI/gpt-j-6b --num_samples 36
"""
...
from paceg_gpt2 import fcg_gradient_gpt2, get_model_tokenizer, _build_base_embed
...
def load_tellmewhy(path: str, num_samples: int, use_gold: bool) -> list[dict]:
    ...
    Returns
    -------
    list of {'question': str, 'gold_answer': str | None, 'target_word': None}
    """
    samples = []
...
def load_longra(path: str, num_samples: int, model, tokenizer, device: str) -> list[dict]:
    ...
    The distractor is assumed to be enclosed in parentheses, e.g.:
        "When my flight landed in Japan, I converted my currency.
         (I had a terrifying dream about my grandmother).
         I was staying in the capital, \t Tokyo"

    Returns
    -------
    list of {'question': str, 'gold_answer': None, 'target_word': str}
    """
    raw_samples = []
    with open(path, "r", encoding="utf-8") as fh:
        for raw in fh:
    ...
    # Filter: keep only samples where the model predicts target_word
    # both WITH and WITHOUT the distractor sentence.
    ...
    for s in raw_samples:
        ...
        # Keep if both predictions match the target word (case-insensitive)
        target_lc = s["target_word"].lower()
        if (target_lc in pred_with.lower() and
                target_lc in pred_without.lower()):
            samples.append({
                "question":    s["prompt"],   # use full prompt WITH distractor
                "gold_answer": None,
...
def load_wikibio(path: str, num_samples: int) -> list[dict]:
    ...
    The model is expected to continue generating the biography.

    Returns
    -------
    list of {'question': str, 'gold_answer': None, 'target_word': None}
    """
    samples = []
    with open(path, "r", encoding="utf-8") as fh:
        for raw in fh:
...
def run_single_example(
    ...
    """
    Run FCGGrad attribution and compute faithfulness metrics for one sample.

    Parameters
    ----------
    sample        : dict with keys 'question', 'gold_answer', 'target_word'
    ...
    stride        : step size for sequence-level evaluation (ignored for 'token')
    """
    res = fcg_gradient_gpt2(
    ...
    # For LongRA token-level: evaluate ONLY at the position of the first
    # generated token (the target word).  Override answer_positions.
...
def run_benchmark(args) -> None:
    ...
    # Register model+tokenizer in the paceg_gpt2 cache so fcg_gradient_gpt2
    # does not reload on every call.
    from paceg_gpt2 import _CACHE as _pg_cache
    _pg_cache[(args.model_name, device)] = (model, tokenizer)
    ...
    for idx, sample in enumerate(tqdm(samples, desc=f"[{experiment}] Evaluating")):
        try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Benchmark FCGGrad Attribution on GPT-family models "
...
""",
    ...
    parser.add_argument(
        ...
        help=(
            "Path to the dataset file.  If omitted, each experiment has a "
            "built-in default path (see EXPERIMENT_META defaults)."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Max samples to evaluate (default: 200; paper uses 200 for TellMeWhy/WikiBio, ~36 for LongRA)",
    )
    parser.add_argument(
        "--steps", type=int, default=100,
        help="Riemann-sum steps for FCG integration (default: 100)",
    )
    parser.add_argument(
        "--max_new_tokens", type=int, default=30,
        help="Max tokens to generate for each answer (default: 30; use ≥5 for WikiBio)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="IG attribution baseline: zero (default) | pad (EOS embedding) | mean (vocab mean)",
    )
    parser.add_argument(
